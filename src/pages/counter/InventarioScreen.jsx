...
export default function InventarioScreen({ inv, zonas, onEntrar, user, deviceId, onLogout }) {
  ...
  const stats = [
    ...
    { label: 'Zonas OK',  value: `${finalizadas}/${zonas.length}`, color: finalizadas === zonas.length ? G : B, bg: finalizadas === zonas.length ? GL : BL },
  ]
    ...
    <div style={{ background: '#F3F4F6', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ margin: '0 14px', background: '#fff', border: '1px solid #E5E7EB', borderTop: `3px solid ${B}` }}>
        <InfoRow
          ...
          label="Sucursal" value={inv.sucursal}
        />
        <InfoRow
          ...
          label="Depósito" value={inv.deposito}
        />
        <InfoRow
      ...
      <div style={{ margin: '12px 14px 0', background: '#fff', border: '1px solid #E5E7EB', padding: 14 }}>
        ...
        <div style={{ marginTop: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 700, color: finalizadas === zonas.length ? G : B, fontFamily: "'DM Mono',monospace" }}>
              {finalizadas}/{zonas.length}
            </span>
          </div>
          <div style={{ background: '#E5E7EB', height: 6 }}>
            <div style={{ height: '100%', width: `${zonas.length ? Math.round(finalizadas / zonas.length * 100) : 0}%`, background: finalizadas === zonas.length ? G : B, transition: 'width .4s' }} />
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: '14px', paddingBottom: 'max(env(safe-area-inset-bottom),14px)', borderTop: '1px solid #E5E7EB', background: '#fff' }}>
        <button
          onClick={onEntrar}
          style={{ width: '100%', padding: '17px 0', background: B, color: '#fff', border: 'none', fontWeight: 700, fontSize: 16, letterSpacing: '0.04em', textTransform: 'uppercase', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}
        >
          <svg width={18} height={18} viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5" strokeLinecap="square">
            <path d="M9 18l6-6-6-6" />
          </svg>
          Iniciar conteo por zonas
        </button>
      </div>

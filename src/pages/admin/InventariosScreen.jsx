  ...
  const handleCerrar = async (id) => {
    if (detalleData) {
      ...
      if (zonasAbiertas.length > 0) {
        alert(`No se puede cerrar el inventario. Hay ${zonasAbiertas.length} zona${zonasAbiertas.length > 1 ? 's' : ''} sin finalizar:\n${zonasAbiertas.map(z => `• ${z.nombre}`).join('\n')}`)
        return
      }
    }
          ...
          >
            ...
            {detalle.estado === 'abierto' && (
              <div style={{ padding: '14px 20px', borderTop: '1px solid #E5E7EB', flexShrink: 0 }}>
                {(() => {
                  const zonasAbiertas = detalleData?.zonas.filter(z => !z.finalizada) || []
                  const bloqueado = zonasAbiertas.length > 0
                  return (
                    <>
                      {bloqueado && (
                        <div style={{ background: '#FFFBEB', border: '1px solid #FDE68A', padding: '8px 12px', marginBottom: 10, fontSize: 12, color: '#92400E', display: 'flex', alignItems: 'center', gap: 8 }}>
                          <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="square" style={{ flexShrink: 0 }}><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1={12} y1={9} x2={12} y2={13}/><line x1={12} y1={17} x2={12.01} y2={17}/></svg>
                          {zonasAbiertas.length} zona{zonasAbiertas.length > 1 ? 's' : ''} sin finalizar
                        </div>
                      )}
                      <button
                        onClick={() => handleCerrar(detalle.id)}
                        disabled={bloqueado}
                        style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px 0', background: bloqueado ? '#F3F4F6' : '#FEF2F2', border: `1px solid ${bloqueado ? '#E5E7EB' : '#FECACA'}`, color: bloqueado ? '#9CA3AF' : '#DC2626', fontSize: 14, fontWeight: 700, cursor: bloqueado ? 'not-allowed' : 'pointer' }}
                      >

...
/**
 * IDs de producto que forman el alcance del inventario (subconjunto del wizard).
 * Devuelve [] si el inventario no tiene subconjunto definido (modo "Todos" → sin
 * restricción). Paginado por si el alcance es grande.
 */
export async function getInventarioProductoIds(inventario_id) {
  const PAGE = 1000
  let all = []
  let from = 0
  while (true) {
    ...
    all = all.concat(data.map(r => Number(r.producto_id)))
